...
function main(){
  const basePokemon = new Pokemon("Charmander", "Fuego", 1, ["Llamarada", "Arañazo"]);
  const pokemonCloke =  basePokemon.clone();
  pokemonCloke.name ='Charizar';
  pokemonCloke.level = 19;
  pokemonCloke.attacks.push("Lanza llamas");

  console.log({basePokemon});
  basePokemon.displayInfo();

  console.log({pokemonCloke})
  pokemonCloke.displayInfo();
