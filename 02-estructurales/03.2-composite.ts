...
class MenuItem implements MenuComponent {
  ...
  private sumaPrice: number;

  constructor(name: string, price: number) {
    this.name = name;
    this.price = price;
    this.sumaPrice = price;
  }

  showDetails(indent: string = ""): void {
    this.sumaPrice = this.price + this.sumaPrice;

    console.log(
      `${indent}- ${this.name}: %c$${this.price.toFixed(2)} `,
      COLORS.green,
      ` $${this.sumaPrice.toFixed(2)} `,
    );
  }
}

//`%c${this.sumaPrice.toFixed(2)}`,COLORS.red
...
class MenuCategory implements MenuComponent {
  ...
  add(item: MenuComponent | MenuComponent[]): void {
    Array.isArray(item) ? this.content.push(...item) : this.content.push(item);
  ...
  showDetails(indent: string = ""): void {
    ...
    this.content.forEach((platillo) => platillo.showDetails(indent + " "));
